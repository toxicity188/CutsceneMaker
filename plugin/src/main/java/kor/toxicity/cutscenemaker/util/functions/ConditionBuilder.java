...
public final class ConditionBuilder<T> {
    ...
    static {
        ...
        LIVING_ENTITY.BOOL.addFunction("potion", new CompileFunction<LivingEntity, Boolean>() {
            @Override
            public FunctionCompiler<LivingEntity, Boolean> initialize() {
                return new FunctionCompiler<LivingEntity, Boolean>() {
                    private PotionEffectType type;
                    @Override
                    public void initialize(JsonArray array) {
                        type = PotionEffectType.getByName(array.get(0).getAsString());
                    }

                    @Override
                    public Function<LivingEntity, Boolean> compile() {
                        return e -> e.hasPotionEffect(type);
                    }
                };
            }
            ...
            public boolean check(JsonArray array) {
                ...
                return PotionEffectType.getByName(array.get(0).getAsString()) != null;
            }
        });
        ...
        LIVING_ENTITY.BOOL.addFunction("hasitem",new CheckableFunction<LivingEntity,Boolean>() {
            ...
            public Boolean apply(LivingEntity entity, JsonArray array) {
                ...
                return InvUtil.has(p,ItemData.getItemStack(p,array.get(0).getAsString()));
            }
        ...
        LIVING_ENTITY.NUMBER.addFunction("storage",(e,j) -> {
            ...
            Player p = (Player) e;
            return InvUtil.storage(p,(j.size() > 0) ? ItemData.getItemStack(p,j.get(0).getAsString()) : null);
        });
        LIVING_ENTITY.NUMBER.addFunction("amount", new CompileFunction<LivingEntity, Number>() {
            ...
            public FunctionCompiler<LivingEntity, Number> initialize() {
                return new FunctionCompiler<LivingEntity, Number>() {
                    ...
                    @Override
                    public void initialize(JsonArray array) {
                        builder = ItemData.getItemBuilder(array.get(0).getAsString());
                    }
